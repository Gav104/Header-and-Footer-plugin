...
//remove slashes to avoid data corruption before sending input data.
	$header = wp_unslash ( get_option ( 'goh_hfp_header_data' ));
	$footer = wp_unslash ( get_option ( 'goh_hfp_footer_data' ));
?>
...
<div class="container" >
  <form action="admin.php?page=Header-and-Footer-plugin/includes/goh-hfp-acp-page.php" method="post">
    ...
    <input value="<?php echo esc_attr ($header) ?>" type="text" name="goh_hfp_header_data" id="header_data" placeholder="<!-- This goes in the header -->">

    <label>Footer Data</label>
    <input value="<?php echo esc_attr ($footer) ?>" type="text" name="goh_hfp_footer_data" id="footer_data" placeholder="<!-- This goes in the footer -->">
